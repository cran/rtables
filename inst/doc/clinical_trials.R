...
lyt6 <- basic_table(show_colcounts = TRUE) %>%
  split_cols_by(var = "ARM") %>%
  split_rows_by("SEX", labels_var = "lbl_sex",
...
ADRS_BESRSPI <- ex_adrs %>%
  ...
  mutate(
      rsp = factor(AVALC %in% c("CR", "PR"),
                   levels = c(TRUE, FALSE), labels = c("Responders", "Non-Responders")),
      is_rsp = (rsp == "Responders")
  )

s_proportion <- function(x, .N_col) {
  in_rows(.list = lapply(as.list(table(x)),
...
rsp_lyt2 <- basic_table(show_colcounts = TRUE) %>%
  split_cols_by("ARMCD", ref_group = "ARM A") %>%
  analyze("rsp", s_proportion, show_labels = "hidden") %>%
  analyze("is_rsp", s_unstrat_resp, show_labels = "visible",
...
a_cph <- function(df, .var, .in_ref_col, .ref_full, full_cox_fit) {
    if (.in_ref_col) {
        ...
        sdf <- survdiff(Surv(AVAL, CNSR == 0) ~ ACTARM + STRATA1,
...
## -----------------------------------------------------------------------------
surv_tbl <- as.data.frame(summary(survfit(Surv(AVAL, CNSR == 0) ~ ACTARM,
                                          data = adtte, conf.type = "log-log"))$table) %>%
    dplyr::mutate(ACTARM = factor(gsub("ACTARM=", "", row.names(.)),
                                  levels = levels(adtte$ACTARM)),
                  ind = FALSE)
